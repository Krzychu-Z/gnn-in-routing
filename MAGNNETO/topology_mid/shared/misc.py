...
def position_raise(edge, pos_map, index):
    link_name = pos_map[index]
...
def readout_raise(readout, r_map, edges):
    for edge in edges:
        for position, each in enumerate(readout):
            ...
            if each > 0:
                position_raise(edge, r_map, position)


def q_learning_raise(q_table, r_map, edges):
    avg_q = np.average(q_table)

    for edge in edges:
        for position, each in enumerate(q_table):
            # We assume readout consists of logit values
            if each > avg_q:
                position_raise(edge, r_map, position)
